...
quiz_trend_data <- quiz %>%
  ...
  mutate(
    quiz_label = factor(paste0("Q", QUIZ_NUMBER),
      levels = paste0("Q", sort(unique(QUIZ_NUMBER)))
    )
  )
#######################################
#######################################
...
student_risk <- quiz %>%
  ...
  summarise(
    ...
    attend_rate    = mean(ATTENDED, na.rm = TRUE)
  ) %>%
  mutate(risk_group = case_when(
    attend_rate < 0.70 ~ "Low Attendance",
    attend_rate >= 0.70 & avg_quiz_score < 12 ~ "High Attendance, Low Score",
    TRUE ~ "On Track"
  ))
# Student #4
ref_student_key <- 1
...
student_skills <- experience %>%
  filter(STUDENT_KEY == ref_student_key) %>%
...
colnames(radar_data) <- c(
  ...
  "Stored Proc", "ETL", "Data Vis", "Req. Gathering"
)
...
server <- function(input, output) {
  ...
  output$inst.total_students <- renderInfoBox({
    total_stu <- length(unique(quiz$STUDENT_KEY))
    infoBox(
      "Total students", 
      total_stu, 
      subtitle = "3 cohorts", 
  ...
  output$inst.avg_attendance <- renderInfoBox({
    avg_att <- mean(quiz$ATTENDED, na.rm = TRUE)
    att_pct <- round(avg_att * 100)
    infoBox(
      "Avg attendance", 
      paste0(att_pct, "%"), 
  ...
  output$inst.avg_score <- renderInfoBox({
    avg_score <- mean(quiz$QUIZ_SCORE, na.rm = TRUE)
    score_pct <- round((avg_score / 20) * 100, 1)
    infoBox(
      "Avg quiz score", 
      paste0(round(avg_score, 1), " / 20"), 
      subtitle = paste0(score_pct, "%"), 
  ...
  output$inst.avg_sentiment <- renderInfoBox({
    avg_sent <- mean(quiz$AVG_T_CLICKER, na.rm = TRUE)
    infoBox(
      "Avg sentiment", 
      paste0(round(avg_sent, 1), " / 5"), 
      subtitle = "Generally positive", 
      icon = icon("smile"), 
      color = "green"
    )
  })
  ...
  output$stu.attendance_plot <- renderPlot({
    ...
    ggplot(attendance_data, aes(x = session, y = height, fill = status)) +
      geom_col(width = 0.4, show.legend = FALSE, linewidth = 0) +
  ...
  # Ins #4
  output$inst.plot4 <- renderPlotly({
    p <- ggplot(student_risk, aes(
      x = attend_rate * 100,
      y = avg_quiz_score,
      color = risk_group,
      text = paste(
        "Student ID:", STUDENT_KEY,
        "<br>Attendance:", paste0(round(attend_rate * 100), "%"),
      ...
      scale_color_manual(values = c(
        "Low Attendance"             = "#E15759",
        "High Attendance, Low Score" = "#F28E2B",
        "On Track"                   = "gray70"
      )) +
  ...
  output$stu.plot4 <- renderPlot({
    radarchart(
      radar_data,
      axistype = 1,
      pcol = c("gray50", "#4E79A7"), # class avg, student
